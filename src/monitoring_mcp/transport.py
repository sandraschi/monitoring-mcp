...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 10809
...
ENV_PORT = "MCP_PORT"  # default: 10851
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10851")),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10851)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10851

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10851 python -m {server_name.replace("-", "_")}
""",
    )
    ...
    parser.add_argument("--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10851)")
